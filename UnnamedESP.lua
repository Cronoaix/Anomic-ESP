...
local LastRefresh = 0
local OptionsFile = 'AnomicESP.json'
local Binding = false
local BindedKey = nil
local OIndex = 0
local LineBox = {}
	 			...
	 			for i, v in pairs(workspace.Entities:GetChildren()) do
					...
					local Properties = v:FindFirstChild'Properties' if not Properties then continue end
	 				...
	 				if Main and Owner and Money then
	 					...
	 					pcall(RenderList.AddOrUpdateInstance, RenderList, v, Main, string.format('%s\nOwned by %s\n[%s]', v.Name, O, M), Color3.fromRGB(1, 255, 51));
	 				end
	 			end
	 		end
	 	end;
...
Options('PrinterEsp', "Printer ESP", true)
